...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C2f_MBRConv5', 'MBRConv3C3', 'MBRConv5C3', 'C2f_VSSD', 'C2f_TVIM', 'DPCF', 'C2f_CSI',
           'CoordAtt_block', 'EMA_block', 'SimAM_block',
           ]
...
class CoordAtt_block(nn.Module):
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.attention = CoordAtt(ch_out) # 注意力机制实例化
    def forward(self, x):
        out = self.branch2a(x)
        out = self.branch2b(out)

        out = self.attention(out) # 注意力机制调用加这行
...
class EMA_block(nn.Module):
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.attention = EMA(ch_out)  # 注意力机制实例化
